...
std::vector<float> content(int kind) { std::vector<float> x(15000); std::uint32_t z = 1234567; for (size_t i=0;i<x.size();++i) { z=1664525*z+1013904223; double t=double(i); x[i]=kind==0?float((int(z>>16)-32768)/163840.0):kind==1?(i%997==0?.8f:0.f):float(.35*sin(t*.013)+.2*sin(t*.071)); } return x; }
Render render(const std::vector<float>& in, double ratio, const std::vector<int>& chunks, SRC_STATE* external=nullptr) {
 int error{}; SRC_STATE* s=external?external:src_new(SRC_SINC_BEST_QUALITY,1,&error); ok(s&&(external||error==0),"src_new"); Render r; size_t offset=0,turn=0;
...
int main(){try{ratio(160,147);ratio(147,160);for(I t:{I(160),I(280),I(4096),I(4256),I(12000)})cacheCheck(160,147,t);for(I t:{I(160),I(236),I(4096),I(4243),I(12000)})cacheCheck(147,160,t);partitions(160,147,4096);partitions(147,160,4096);invalidation(160,147,4096);invalidation(147,160,4096);edits(160,147);edits(147,160);quality(160,147,44100,48000);quality(147,160,48000,44100);std::cout<<"OB-LSR LS5-LS22 PASS phases=307 cache=8224 callback=0 backend=partially-unproven\n";return 0;}catch(const std::exception&e){std::cerr<<"OB-LSR FAIL "<<e.what()<<'\n';return 1;}}
