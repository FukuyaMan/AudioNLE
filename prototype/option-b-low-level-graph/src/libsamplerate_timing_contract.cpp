...
namespace {
using I = std::int64_t;
constexpr I nominal = 1024, pageFrames = 257, pageCount = 9;
...
double amplitude(const Render&r,double frequency,double rate){const I begin=1000,n=6000;ok(r.x.size()>size_t(begin+n),"quality coverage");double s{},c{};for(I i=0;i<n;++i){double a=2.0*3.141592653589793*frequency*double(begin+i)/rate;s+=r.x[size_t(begin+i)]*sin(a);c+=r.x[size_t(begin+i)]*cos(a);}return 2*sqrt(s*s+c*c)/n;}
...
int main(int argc,char**argv){try{if(argc==2&&std::string_view(argv[1])=="--medium"){converter=SRC_SINC_MEDIUM_QUALITY;converterName="SRC_SINC_MEDIUM_QUALITY";}else if(argc==2&&std::string_view(argv[1])=="--fastest"){converter=SRC_SINC_FASTEST;converterName="SRC_SINC_FASTEST";}for(const auto& r:std::array<std::array<I,2>,6>{{{{160,147}},{{147,160}},{{320,147}},{{147,320}},{{2,1}},{{1,2}}}})ratio(r[0],r[1]);for(const auto& r:std::array<std::array<I,2>,6>{{{{160,147}},{{147,160}},{{320,147}},{{147,320}},{{2,1}},{{1,2}}}}){cacheCheck(r[0],r[1],4096);partitions(r[0],r[1],4096);invalidation(r[0],r[1],4096);edits(r[0],r[1]);}quality(160,147,44100,48000);quality(147,160,48000,44100);quality(320,147,44100,96000);quality(147,320,96000,44100);quality(2,1,48000,96000);quality(1,2,96000,48000);std::cout<<"OB-LSR mode="<<converterName<<" 96k V1 PASS cache="<<pageCount*pageFrames*I(sizeof(float))<<" callback=0 backend=partially-unproven\n";return 0;}catch(const std::exception&e){std::cerr<<"OB-LSR FAIL "<<e.what()<<'\n';return 1;}}
